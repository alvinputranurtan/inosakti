        ...
        <?php if ($isModule && isset($moduleKind) && $moduleKind === 'powerpoint'): ?>
          <div class="mt-4 rounded-xl border border-slate-200 bg-white p-4 space-y-3">
            <div class="font-semibold text-sm text-slate-700">Presentasi/PDF dari Repository</div>
            <form method="post" class="grid lg:grid-cols-[1fr_auto] gap-2 items-end">
              <input type="hidden" name="csrf_token" value="<?= admin_e(admin_csrf_token()) ?>">
              ...
              <input type="hidden" name="module_lesson_id" value="<?= (int) ($selectedModuleLesson['id'] ?? 0) ?>">
              <div>
                <label class="block text-xs font-semibold mb-1">Pilih File Presentasi/PDF</label>
        ...
        <option value="">-- Pilih file dari repository --</option>
            ...
            <form method="post" class="grid lg:grid-cols-[1fr_auto] gap-2 items-end">
              ...
              <div class="grid lg:grid-cols-2 gap-2">
                <div>
                  <label class="block text-xs font-semibold mb-1">File Presentasi/PDF</label>
            ...
            <form method="post" onsubmit="return confirm('apakah benar anda ingin menghapus file presentasi/PDF ini dari repository?');" class="grid lg:grid-cols-[1fr_auto] gap-2 items-end">
              <input type="hidden" name="csrf_token" value="<?= admin_e(admin_csrf_token()) ?>">
